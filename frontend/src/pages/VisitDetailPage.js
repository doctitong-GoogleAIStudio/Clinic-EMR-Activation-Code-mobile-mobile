            ...
            <CardContent>
              {soapAttachments.length === 0 ? (
                <div className="text-center py-8 text-slate-400">
                  ...
                  {isDoctor && <p className="text-xs mt-1">Click "Upload File" to add scanned notes or documents</p>}
                </div>
              ) : (
                <div className="space-y-3">
                  {soapAttachments.map((att) => (
                    ...
                    >
                      {editingSoapAttachment?.id === att.id ? (
                        // Edit mode
                        <div className="flex-1 space-y-2">
                          <Input
                            value={soapAttachmentForm.filename}
                            onChange={(e) => setSoapAttachmentForm({ ...soapAttachmentForm, filename: e.target.value })}
                            placeholder="Filename"
                            className="h-8"
                            data-testid="soap-edit-filename"
                          />
                          <Input
                            value={soapAttachmentForm.notes}
                            onChange={(e) => setSoapAttachmentForm({ ...soapAttachmentForm, notes: e.target.value })}
                            placeholder="Notes (optional)"
                            className="h-8"
                            data-testid="soap-edit-notes"
                          />
                          <div className="flex gap-2">
                            <Button size="sm" onClick={saveSoapAttachmentEdit} className="bg-[#0F766E] hover:bg-[#115E59]">
                              Save
                            </Button>
                            <Button size="sm" variant="ghost" onClick={() => setEditingSoapAttachment(null)}>
                              Cancel
                            </Button>
                          </div>
                        </div>
                        ...
                        <>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-400">{format(parseISO(att.uploaded_at), 'MMM d, h:mm a')}</p>
                            </div>
                          </div>
                            ...
                            {isDoctor && (
                              <>
                                <Button 
                                  variant="ghost" 
                                  size="sm" 
                                  onClick={() => handleEditSoapAttachment(att)}
                                  className="h-8 w-8 p-0 text-slate-500 hover:text-[#0F766E]"
