...
ED25519_PUBLIC_KEY_B64 = os.environ.get('ED25519_PUBLIC_KEY')
LICENSE_MASTER_SECRET = os.environ.get('LICENSE_MASTER_SECRET')
LICENSE_SUPER_ADMIN_EMAIL = os.environ.get('LICENSE_SUPER_ADMIN_EMAIL')
LICENSE_SUPER_ADMIN_PASSWORD = os.environ.get('LICENSE_SUPER_ADMIN_PASSWORD')
...
class LicenseGenerateRequest(BaseModel):
    ...
    license_type: LicenseType = LicenseType.LIFETIME
    expires_at: Optional[str] = None  # ISO date string
    trial_days: Optional[int] = 30
    trial_patient_limit: Optional[int] = 100
    app_name: str = "Private Clinic EMR"
    notes: Optional[str] = None
...
class LicenseExtendRequest(BaseModel):
    new_expires_at: Optional[str] = None  # ISO date for yearly
...
def _get_signing_key():
    """Load Ed25519 signing key from env."""
    key_bytes = base64.b64decode(ED25519_PRIVATE_KEY_B64)
    return nacl.signing.SigningKey(key_bytes)

def _get_verify_key():
    """Load Ed25519 verify key from env."""
    key_bytes = base64.b64decode(ED25519_PUBLIC_KEY_B64)
    return nacl.signing.VerifyKey(key_bytes)

def generate_activation_code(device_id: str, license_type: str, app_name: str) -> str:
    """Generate a short activation code using HMAC-SHA256."""
    payload = f"{device_id}|{app_name}|{license_type}"
    h = hmac.new(LICENSE_MASTER_SECRET.encode(), payload.encode(), hashlib.sha256).digest()
    code = h[:8].hex().upper()
    return f"DDH-{code[:4]}-{code[4:8]}-{code[8:12]}-{code[12:16]}"

def sign_license_payload(payload_str: str) -> str:
    """Sign a license payload string with Ed25519 private key, return base64 signature."""
    signing_key = _get_signing_key()
    signed = signing_key.sign(payload_str.encode())
    return base64.b64encode(signed.signature).decode()

def verify_license_signature(payload_str: str, signature_b64: str) -> bool:
    """Verify an Ed25519 signature."""
    try:
        verify_key = _get_verify_key()
        signature = base64.b64decode(signature_b64)
        verify_key.verify(payload_str.encode(), signature)
        return True
    except Exception:
        return False
...
def compute_license_check(license_data: dict) -> dict:
    """Check license status including grace period."""
    now = datetime.now(timezone.utc)
    status = license_data.get("status", "active")
    license_type = license_data.get("license_type", "lifetime")
    ...
    if status == "revoked":
        result["message"] = "License has been revoked. Contact your administrator."
        return result
    
    if license_type == "lifetime" or license_type == "hospital":
        result["is_valid"] = True
        result["status"] = "active"
        result["message"] = "License is active (Lifetime)."
        return result
    ...
    if expires_at_str:
        ...
        if now < expires_at:
            ...
            result["message"] = f"License active. {days_left} days remaining."
        else:
            # Grace period: 7 days after expiry
            grace_end = expires_at + timedelta(days=7)
            if now < grace_end:
                result["is_valid"] = True
                result["in_grace_period"] = True
                result["status"] = "grace_period"
                result["grace_days_remaining"] = (grace_end - now).days
                result["message"] = f"License expired! {result['grace_days_remaining']} grace days remaining."
            else:
                result["status"] = "expired"
                result["message"] = "License expired and grace period ended. Contact your administrator."
    else:
        result["is_valid"] = True
        result["status"] = "active"
...
@api_router.post("/license/admin/login")
async def super_admin_login(creds: SuperAdminLogin):
    if creds.email != LICENSE_SUPER_ADMIN_EMAIL or creds.password != LICENSE_SUPER_ADMIN_PASSWORD:
        raise HTTPException(status_code=401, detail="Invalid super admin credentials")
    token = create_super_admin_token()
    return {"token": token, "role": "super_admin", "email": creds.email}

# --- License Generation (Super Admin) ---

@api_router.post("/license/admin/generate")
async def generate_license(req: LicenseGenerateRequest, admin=Depends(get_super_admin)):
    # Check if device already has an active license
    existing = await db.licenses.find_one({"device_id": req.device_id, "status": {"$in": ["active", "grace_period"]}})
    if existing:
        raise HTTPException(status_code=400, detail="Device already has an active license. Revoke it first.")
    
    activation_code = generate_activation_code(req.device_id, req.license_type, req.app_name)
    
    # Determine expiry
    expires_at = None
    if req.license_type == LicenseType.YEARLY:
        if req.expires_at:
            expires_at = req.expires_at
        else:
            expires_at = (datetime.now(timezone.utc) + timedelta(days=365)).isoformat()
    elif req.license_type == LicenseType.TRIAL:
        trial_days = req.trial_days or 30
        expires_at = (datetime.now(timezone.utc) + timedelta(days=trial_days)).isoformat()
    
    # Create signed payload for offline verification
    ...
    license_doc = {
        ...
        "expires_at": expires_at,
        "trial_patient_limit": req.trial_patient_limit if req.license_type == LicenseType.TRIAL else None,
        "signature": signature,
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": req.device_id,
        "action": "generated",
        "details": f"License generated for {req.customer_name} ({req.license_type})",
    ...
    return {
        ...
        "expires_at": expires_at,
        "customer_name": req.customer_name,
        "offline_key": base64.b64encode(json.dumps({
...
async def activate_license(req: LicenseActivateRequest):
    """Activate a device with activation code (online mode)."""
    ...
    if not license_doc:
        raise HTTPException(status_code=400, detail="Invalid activation code for this device.")
    ...
    if license_doc["status"] == "active":
        # Already activated — return license data
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": req.device_id,
        "action": "activated",
        "details": f"Device activated ({license_doc['license_type']})",
        "performed_by": "device",
        "timestamp": now
    })
    
    check = compute_license_check({**license_doc, "status": "active"})
...
@api_router.post("/license/activate-offline")
async def activate_offline(device_id: str = Form(...), offline_key: str = Form(...)):
    """Activate using an offline key (base64 encoded signed payload)."""
    try:
        decoded = json.loads(base64.b64decode(offline_key))
        payload_str = decoded["payload"]
        sig = decoded["signature"]
        activation_code = decoded["activation_code"]
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid offline key format.")
    
    # Verify signature
    if not verify_license_signature(payload_str, sig):
    ...
    if payload.get("device_id") != device_id:
        raise HTTPException(status_code=400, detail="This activation belongs to another device.")
    
    check = compute_license_check({
        "status": "active",
        "license_type": payload.get("license_type", "lifetime"),
...
async def verify_license(device_id: str = Form(...), signature: str = Form(...), signed_payload: str = Form(...)):
    """Verify an existing license. Can work offline with Ed25519 public key."""
    # Verify signature
    if not verify_license_signature(signed_payload, signature):
        raise HTTPException(status_code=400, detail="Invalid license signature.")
    
    payload = json.loads(signed_payload)
    if payload.get("device_id") != device_id:
        raise HTTPException(status_code=400, detail="License does not match this device.")
    
    # Check server-side status (if available)
    server_license = await db.licenses.find_one({"device_id": device_id, "status": {"$ne": "revoked"}}, {"_id": 0})
    
    if server_license and server_license.get("status") == "revoked":
        return {"is_valid": False, "status": "revoked", "message": "License has been revoked."}
    
    # Use the license data for check
    license_data = server_license if server_license else {
        "status": "active",
        "license_type": payload.get("license_type"),
        "expires_at": payload.get("expires_at")
    }
    
    check = compute_license_check(license_data)
    return check

# --- Super Admin: List All Licenses ---

@api_router.get("/license/admin/licenses")
async def list_licenses(admin=Depends(get_super_admin)):
    licenses = await db.licenses.find({}, {"_id": 0}).sort("created_at", -1).to_list(500)
    # Compute check for each
    for lic in licenses:
        lic["check"] = compute_license_check(lic)
    return {"licenses": licenses}

# --- Super Admin: Get Single License ---

@api_router.get("/license/admin/licenses/{license_id}")
async def get_license(license_id: str, admin=Depends(get_super_admin)):
...
# --- Super Admin: Revoke License ---

@api_router.put("/license/admin/revoke/{license_id}")
async def revoke_license(license_id: str, admin=Depends(get_super_admin)):
    ...
    await db.license_audit_logs.insert_one({
        ...
        "action": "revoked",
        "details": f"License revoked for {lic.get('customer_name', 'unknown')}",
        "performed_by": "super_admin",
        "timestamp": datetime.now(timezone.utc).isoformat()
    })
    
    return {"message": "License revoked", "license_id": license_id}

# --- Super Admin: Reactivate License ---

@api_router.put("/license/admin/reactivate/{license_id}")
async def reactivate_license(license_id: str, admin=Depends(get_super_admin)):
    lic = await db.licenses.find_one({"id": license_id})
    if not lic:
        raise HTTPException(status_code=404, detail="License not found")
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": lic["device_id"],
        "action": "reactivated",
        "details": f"License reactivated for {lic.get('customer_name', 'unknown')}",
        "performed_by": "super_admin",
        "timestamp": datetime.now(timezone.utc).isoformat()
    })
    
    return {"message": "License reactivated", "license_id": license_id}

# --- Super Admin: Extend License ---

@api_router.put("/license/admin/extend/{license_id}")
async def extend_license(license_id: str, req: LicenseExtendRequest, admin=Depends(get_super_admin)):
    lic = await db.licenses.find_one({"id": license_id})
    if not lic:
        raise HTTPException(status_code=404, detail="License not found")
    ...
    if req.new_expires_at:
        new_expires = req.new_expires_at
    elif req.additional_days:
        current_exp = lic.get("expires_at")
        base = datetime.fromisoformat(current_exp.replace('Z', '+00:00')) if current_exp else datetime.now(timezone.utc)
        new_expires = (base + timedelta(days=req.additional_days)).isoformat()
    
    if not new_expires:
        raise HTTPException(status_code=400, detail="Provide new_expires_at or additional_days")
    
    # Re-sign the payload
    old_payload = json.loads(lic.get("signed_payload", "{}"))
    old_payload["expires_at"] = new_expires
    new_payload_str = json.dumps(old_payload, sort_keys=True)
    new_signature = sign_license_payload(new_payload_str)
    new_code = generate_activation_code(lic["device_id"], lic["license_type"], lic.get("app_name", "Private Clinic EMR"))
    ...
    await db.licenses.update_one(
        ...
        {"$set": {
            ...
            "status": "active",
            "signed_payload": new_payload_str,
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": lic["device_id"],
        "action": "extended",
        "details": f"Extended to {new_expires}",
        "performed_by": "super_admin",
        "timestamp": datetime.now(timezone.utc).isoformat()
    })
    
    return {"message": "License extended", "new_expires_at": new_expires, "new_activation_code": new_code}

# --- Super Admin: Transfer License ---

@api_router.put("/license/admin/transfer/{license_id}")
async def transfer_license(license_id: str, req: LicenseTransferRequest, admin=Depends(get_super_admin)):
    lic = await db.licenses.find_one({"id": license_id})
    if not lic:
        raise HTTPException(status_code=404, detail="License not found")
    
    old_device = lic["device_id"]
    
    # Re-sign for new device
    old_payload = json.loads(lic.get("signed_payload", "{}"))
    old_payload["device_id"] = req.new_device_id
    new_payload_str = json.dumps(old_payload, sort_keys=True)
    new_signature = sign_license_payload(new_payload_str)
    new_code = generate_activation_code(req.new_device_id, lic["license_type"], lic.get("app_name", "Private Clinic EMR"))
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": req.new_device_id,
        "action": "transferred",
        "details": f"Transferred from {old_device} to {req.new_device_id}",
        "performed_by": "super_admin",
        "timestamp": datetime.now(timezone.utc).isoformat()
    })
    
    return {"message": "License transferred", "new_device_id": req.new_device_id, "new_activation_code": new_code}

# --- Super Admin: Delete License ---

@api_router.delete("/license/admin/licenses/{license_id}")
async def delete_license(license_id: str, admin=Depends(get_super_admin)):
    ...
    await db.license_audit_logs.insert_one({
        ...
        "device_id": "N/A",
        "action": "deleted",
        "details": "License record deleted",
        "performed_by": "super_admin",
        "timestamp": datetime.now(timezone.utc).isoformat()
    })
...
@api_router.get("/license/admin/audit")
async def get_license_audit(license_id: Optional[str] = None, admin=Depends(get_super_admin)):
    query = {}
    if license_id:
        query["license_id"] = license_id
    logs = await db.license_audit_logs.find(query, {"_id": 0}).sort("timestamp", -1).to_list(200)
    return {"logs": logs}

# --- Public Key Endpoint (for offline verification) ---

@api_router.get("/license/public-key")
...
async def license_stats(admin=Depends(get_super_admin)):
    total = await db.licenses.count_documents({})
    active = await db.licenses.count_documents({"status": "active"})
    pending = await db.licenses.count_documents({"status": "pending"})
    revoked = await db.licenses.count_documents({"status": "revoked"})
    expired = await db.licenses.count_documents({"status": "expired"})
    
    by_type = {}
    for lt in ["lifetime", "yearly", "trial", "hospital"]:
        by_type[lt] = await db.licenses.count_documents({"license_type": lt})
...
async def startup():
    ...
    # License indexes
    await db.licenses.create_index("device_id")
    await db.licenses.create_index("activation_code")
    await db.licenses.create_index("status")
    await db.license_audit_logs.create_index("license_id")
    await db.license_audit_logs.create_index("timestamp")
